...
module.exports.readConfig = ({ cwd, data }) => {
  ...
  let settingPrompts = Object.entries(settingConfig).map(
    ([key, value], index) => {
      let prompts = {
        group: "global",
        name: key,
  ...
  let [devConfig, proConfig, stageConfig] = [devData, proData, stageData].map(
    (source) => {
      let env = [],
        note = "";
      source
  ...
  let [devPrompts, proPrompts, stagePrompts] = [
    devConfig,
    proConfig,
    stageConfig,
  ].map((config, j) => {
  ...
  return {
    tabs: [
      ...
      },
      {
        ...
        prompts: [...devPrompts, ...proPrompts, ...stagePrompts],
      },
    ],
  };
};
