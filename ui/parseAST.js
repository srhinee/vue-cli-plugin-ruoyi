...
function walkObject (obj, source) {
  ...
  properties.forEach (n => {
      ...
      if (type === "Literal") {//assign
        result[name] = n.value.value
      } else if (type === "ObjectExpression") { //object
        result[name] = walkObject (n.value, source)
      } else if (type === "Identifier") { //variable
        result[name] = n.value.name
      } else if (type === "TemplateLiteral") {
        let {expressions, quasis} = n.value
        result[name] = "";
        [...expressions, ...quasis].sort ((a, b) => a.start - b.start).forEach (v => {
...
function walkFn (node, type, source) {
  try {
    ...
    ["data", "headers", "params"].forEach (v => {
      if (item[v] && typeof item[v] === "string") item[v] = {[item[v]]: item[v]}
    })
